...
class ujian_siswa extends CI_Controller {
	...
	//jika ada soal
            ...
            $ambil_data3 = $this->db->query("SELECT soal_ujian.`id_soal`, soal_ujian.`pertanyaan`, hasil_ujian.`kunci_jawaban` FROM soal_ujian JOIN hasil_ujian ON soal_ujian.`id_soal` = hasil_ujian.`id_soal` WHERE nis='$nis' ORDER BY id_soal ASC;");
            $total_data = $ambil_data3->num_rows();
            $id_soal = $total_data + 1;

			$query2= "SELECT * FROM soal_ujian WHERE id_soal=$id_soal ORDER BY id_soal ASC LIMIT 1";
    ...
    {    
        ...
        for ($i=1; $i<=$jumlah; $i++){
            ...
            if($nomor != 0 AND $jawaban !=""){
                $masukkan_hasil_ujian= $this->db->query("INSERT INTO hasil_ujian (id_ujian, nis, id_soal, kunci_jawaban, nilai_cosine) VALUES ('','$nis', '$nomor', '$jawaban','')");
            }
        ...
        foreach ($ambil_jumlah5->result_array() as $row) {
            ...
            foreach ($ambil_hasil_akar->result_array() as $row) {
                ...
                //if($no_soal == )
                $masukkan_cosine=$this->db->query("INSERT INTO hasil_cosine VALUES ('$id_soal','$bulatkan5')");
            }
        }
